...
class Controller:
	...
	def get_mode(self):
		...
		# result = self.client.read_holding_registers(self.OPR_MODE, 1 )
	...
	def get_rpm(self):
		...
		# rpms = self.client.read_holding_registers(self.L_FB_RPM, 2 )
		...
		fb_L_rpm = np.frombuffer(np.uint16(registers[0]).tobytes(), dtype=np.int16)[0] / 10.0

		fb_R_rpm = np.frombuffer(np.uint16(registers[1]).tobytes(), dtype=np.int16)[0] / 10.0
	...
	def set_maxRPM_pos(self, max_L_rpm, max_R_rpm):
		...
		result = self.client.write_registers(self.L_MAX_RPM_POS, [int(max_L_rpm), int(max_R_rpm)] )

	def set_position_async_control(self):

		result = self.client.write_register(self.POS_CONTROL_TYPE, self.ASYNC )

	def move_left_wheel(self):

		result = self.client.write_register(self.CONTROL_REG, self.POS_L_START )

	def move_right_wheel(self):

		result = self.client.write_register(self.CONTROL_REG, self.POS_R_START )
	...
	def set_relative_angle(self, ang_L, ang_R):
		...
		result = self.client.write_registers(self.L_CMD_REL_POS_HI, all_cmds_array )
	...
	def get_wheels_travelled(self):
		...
		# 	result = self.client.read_holding_registers(self.L_FB_POS_HI, 4 )
		# 	try:
